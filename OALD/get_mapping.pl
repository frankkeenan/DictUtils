...
#   $opt_L = ""; # name of file for the log_fp output to go to
    ...
    printf("\#Oldid\tNew eid\tNew e_eid\n"); 
  line:    
    while (<>){
...
#	next line if (m|<entry[^>]*sup=\"y|io);
	...
	my $e_eid = restructure::get_tag_attval($_, "entry", "e:id");
	$_ =~ s|(<[^>]* e:id=\".*?\"[^>]*>)|&split;&fk;$1&split;|gi;
	my @BITS = split(/&split;/, $_);
	my $res = "";
	foreach my $bit (@BITS){
	    if ($bit =~ s|&fk;||gi){
		my $tagname = restructure::get_tagname($bit);
		my $oldid = restructure::get_tag_attval($bit, $tagname, "sourceid");
		my $newid = restructure::get_tag_attval($bit, $tagname, "e:id");
		printf("%s\t%s\t%s\n", $oldid, $newid, $e_eid); 
	    }
#	    $res .= $bit;
	}
