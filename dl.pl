...
#   $opt_L = ""; # name of file for the log_fp output to go to
    ...
    my $comm = sprintf("curl -s --user frank.keenan:%s \"https://dws-dps.idm.fr/api/v1/projects/%s/entries/export/allInternalAttributesAndAdditionalMetadata\"  | perl  ./add_missing_end_tags.pl  > %s/dps.xml", $DPSPASS, $opt_c, $resdir); 
    printf(STDERR "%s\n\n", $comm);
    unless ($opt_d)
    {
	system($comm);
