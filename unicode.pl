#!/usr/local/bin/perl
# require "/home/keenanf/perl/unicode.pl"
&load_phon_unicode;
...
sub phon_ents
{
	...
	if ($bit =~ s|&fk;||gio)
	{
	    ...
	    $bit =~ s|</?wb *>||gio;
#	    if ($bit =~ m|^(.*>)(.*?)(</[iy]>)|io)
	    if ($bit =~ m/^(.*>)(.*?)(<\/(i|y|phon-gb|phon-us|eph)>)/io)
	    {
		$otag = $1;
		$phon = $2;
		$ctag = $3;		
		$phon =~ s|&shy;||gio;
		$phon =~ s|&\#x2027;|&middot;|gi;
		$phon =~ s|&middot;|£|gio;
		$phon =~ s|&nbhyph.*?;|-|gio;
		$phon =~ s|&#x0026;|&|gio;
		$phon =~ s|&#x0303;|~|gio;
...
sub do_ents
{
    ...
    $e =~ s|&pipe;|&\#x007C;|go;
    $e =~ s|&s;|&\#x02CC;|go;
    $e =~ s|&sstress;|&\#x02CC;|go;
    $e =~ s|&sol;|/|goi;
    $e =~ s|&tilde;|~|go;
    $e =~ s|&w;|&\#x2027;|go;
    
    # try and speed it up with going back unless there are some still left
    unless ($e =~ m|&[a-z]|i)
    {
	return $e;
    }
